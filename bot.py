...
# Setup logging to catch internal errors in Railway "Deploy Logs"
...
class CarouselView(discord.ui.View):
    ...
    def get_embed(self) -> discord.Embed:
        if not self.tracks: return discord.Embed(title="No preview links available")
...
class WebhookBot(commands.Bot):
    ...
    async def handle_webhook(self, request: web.Request):
        ...
        if request.headers.get('X-Webhook-Secret') != WEBHOOK_SECRET:
            logger.warning("Unauthorized 401 Attempt")
        ...
        try:
            ...
            # 🔍 Fetch User Profile for "Mental Prep"
            profile = {"full_name": "Unknown", "avatar_url": None}
            ...
            view.add_item(discord.ui.Button(label="Admin Board", url="https://song-tailor.vercel.app/admin", style=discord.ButtonStyle.link))
            ...
            logger.error(f"CRASH: {str(e)}") # This shows the error in Railway HTTP Logs
